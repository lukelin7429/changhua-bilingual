/* =====================================================================
   Nanhsing Elementary — motion engine  (motion.js)
   - scroll progress bar + back-to-top
   - hero entrance + drifting ambient orbs (blue / gold / earth)
...
(function(){
  ...
  ready(function(){
    ...
    var hero=document.querySelector('.page-banner,.unit-hero,.title,.banner,.th-fest-hero');
    if(hero){
      if(getComputedStyle(hero).position==='static') hero.style.position='relative';

      var inner=hero.querySelector(':scope > .page-banner__inner, :scope > .wrap, :scope > .banner__inner, :scope > .th-fest-hero__inner') || hero;
      var kids=[].slice.call(inner.children);          /* capture BEFORE adding orbs */
      inner.classList.add('mo-hero');
      kids.forEach(function(c,i){ c.style.animationDelay=(0.16+i*0.14)+'s'; });

      var orbs=document.createElement('div');
      orbs.className='mo-orbs';
      [
        {s:300,t:'-9%', l:'-6%', c:'rgba(92,147,214,.6)',  dx:'40px',  dy:'32px',  d:17},
        {s:230,t:'30%', l:'82%', c:'rgba(224,163,42,.55)', dx:'-46px', dy:'26px',  d:21},
        {s:170,t:'62%', l:'12%', c:'rgba(207,107,74,.5)',  dx:'30px',  dy:'-34px', d:14},
        {s:140,t:'8%',  l:'58%', c:'rgba(221,233,248,.6)', dx:'-26px', dy:'30px',  d:19}
      ].forEach(function(o){
    ...
    /* ---------- SCROLL-REVEAL ---------- */
    var SEL=['.sec__no','.sec__title','.sec__rule','.about__lead','.about__body',
      '.pillar','.principal-card','.contact','.card','.lcard','.unit','.vc','.course',
      '.playlist-block','.news-grid > a','.gallery img','.cta','.feature','.info-card',
      '.word','.turn','.topic','.sow','.sow-item','.lesson','.quote','.gate-figure',
      '.bigidea','.tip','.video-frame','.dialog__scene',
    ...
    var CALM={vc:1,vocab:1,result:1,turn:1,'sow-item':1,topic:1};   /* dense lists -> gentle up only */
    ...
    function reveal(el){
      ...
      el.style.transitionDelay=(Math.max(0,sibs.indexOf(el))*75)+'ms';
    ...
    document.addEventListener('click',function(ev){
      var b=ev.target.closest && ev.target.closest('.say,.speak,.sow__say,.ln__speak');
      if(!b) return;
