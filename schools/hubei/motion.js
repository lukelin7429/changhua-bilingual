...
(function(){
  ...
  ready(function(){
    ...
    var hero=document.querySelector('.title,.hero,.po-hero,.page-banner,.th-fest-hero,.unit-hero');
    if(hero){
      if(getComputedStyle(hero).position==='static') hero.style.position='relative';

      var inner=hero.querySelector(':scope > .wrap, :scope > .th-fest-hero__inner') || hero;
      ...
      [
        {s:300,t:'-10%',l:'-6%', c:'rgba(59,149,199,.55)', dx:'38px',  dy:'30px',  d:17},
        {s:220,t:'30%', l:'82%', c:'rgba(76,174,126,.50)', dx:'-44px', dy:'26px',  d:21},
        {s:160,t:'64%', l:'14%', c:'rgba(212,165,46,.52)', dx:'30px',  dy:'-34px', d:14}
      ].forEach(function(o){
    ...
    var SEL=['.section-head','.about-text','.timeline__row','.pillar','.principal-card',
      '.life-card','.contact','.card','.th-fcard','.fest-card','.lcard','.unit','.vc','.course',
      '.trad','.scn','.ag','.vocab','.result','.playlist-block','.player','.credits',
      '.topics','.offices','.closing','.news-grid > a',
      '.gallery img','.unit__word','.sec__head','.intro','.cta','.feature','.info-card',
    ...
    var CALM={timeline__row:1,trad:1,vocab:1,result:1,topic:1};   /* dense lists -> gentle up only */
    ...
    document.addEventListener('click',function(ev){
      var b=ev.target.closest && ev.target.closest('.speak,.ln__speak,.say');
      if(!b) return;
