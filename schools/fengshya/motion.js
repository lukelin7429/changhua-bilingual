/* =====================================================================
   Fengshya Elementary — shared motion engine  (motion.js)
...
(function(){
  ...
  ready(function(){
    ...
    var hero=document.querySelector('.title,.page-banner,.banner,.hero,.strip,.th-fest-hero,.unit-hero');
    if(hero){
      ...
      [
        {s:280,t:'-8%', l:'-5%', c:'rgba(204,74,24,.42)',  dx:'36px',  dy:'30px',  d:17},
        {s:210,t:'34%', l:'80%', c:'rgba(200,146,10,.40)', dx:'-42px', dy:'24px',  d:21},
        {s:150,t:'66%', l:'16%', c:'rgba(42,107,42,.34)',  dx:'28px',  dy:'-32px', d:14}
      ].forEach(function(o){
    ...
    var SEL=['.section-head','.about-text','.timeline__row','.pillar','.principal-card',
      '.life-card','.contact','.card','.th-fcard','.lcard','.unit','.vc','.course',
      '.trad','.scn','.ag','.vocab','.result','.playlist-block','.news-grid > a',
      '.gallery figure','.gallery img','.unit__word','.sec__head','.intro','.cta','.feature','.info-card','.prog-card','.step','.chip','.phrase','.prose',
    ...
    document.addEventListener('click',function(ev){
      var b=ev.target.closest && ev.target.closest('.speak,.ln__speak');
      if(!b) return;
