/* =====================================================================
   Guangxing Elementary — site-wide motion engine  (motion.js)
   Tuned copy of the Hub motion engine: recognizes Guangxing's banner
   classes (.pbanner / .unit-banner / .hero-img / .th-fest-hero / .hero)
   and its content blocks. Loaded on EVERY Guangxing page.
...
(function(){
  ...
  ready(function(){
    ...
    var hero=document.querySelector(
      '.pbanner,.unit-banner,.hero-img,.th-fest-hero,.hero,.banner,.page-banner');
    if(hero){
      if(getComputedStyle(hero).position==='static') hero.style.position='relative';
      if(getComputedStyle(hero).overflow==='visible') hero.style.overflow='hidden';

      var inner=hero.querySelector(
        ':scope > .bn, :scope > .wrap, :scope > .hero-cap, '+
        ':scope > .banner__inner, :scope > .hero-inner, '+
