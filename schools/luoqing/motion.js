/* =====================================================================
   Luoqing Elementary — shared motion engine  (motion.js)
...
(function(){
  ...
  ready(function(){
    ...
    var hero=document.querySelector('.banner,.hero,.th-fest-hero,.strip,.unit-hero,.page-banner,.title');
    if(hero){
      if(getComputedStyle(hero).position==='static') hero.style.position='relative';

      var inner=hero.querySelector(':scope > .banner__inner, :scope > .hero__inner, :scope > .th-fest-hero__inner, :scope > .wrap') || hero;
      ...
      [
        {s:280,t:'-8%', l:'-5%', c:'rgba(232,138,60,.50)', dx:'36px',  dy:'30px',  d:17},
        {s:210,t:'34%', l:'80%', c:'rgba(43,86,135,.50)',  dx:'-42px', dy:'24px',  d:21},
        {s:150,t:'66%', l:'16%', c:'rgba(74,138,74,.50)',  dx:'28px',  dy:'-32px', d:14}
      ].forEach(function(o){
    ...
    /* ---------- SCROLL-REVEAL ---------- */
    var SEL=['.sec__no','.about__lead','.about__body > p','.spot','.program','.tl-item',
      '.nc','.fact','.pillar','.quote','.contact','.timeline','.spots','.programs',
      '.lcard','.unit','.vc','.course','.card','.feature','.info-card','.th-fcard',
      '.life-card','.vocab','.result','.scn','.ag','.trad','.playlist-block',
      '.news-row > a','.news-grid > a','.gallery img','.cta','.intro',
    ...
    var CALM={'tl-item':1,trad:1,vocab:1,result:1};   /* dense lists -> gentle up only */
    ...
    document.addEventListener('click',function(ev){
      var b=ev.target.closest && ev.target.closest('.speak,.say,.ln__speak,.SPEAK');
      if(!b) return;
